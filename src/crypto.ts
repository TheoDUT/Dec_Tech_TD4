...
export async function exportPubKey(key: webcrypto.CryptoKey): Promise<string> {
  let exportedKey = await webcrypto.subtle.exportKey("spki", key);
...
export async function importPubKey(strKey: string): Promise<webcrypto.CryptoKey> {
  ...
  return await webcrypto.subtle.importKey("spki", 
    keyBuffer,
    { name: "RSA-OAEP",
      hash: "SHA-256"},
    true, ["encrypt"]);
...
export async function importPrvKey(strKey: string): Promise<webcrypto.CryptoKey> {
  const keyBuffer = base64ToArrayBuffer(strKey);
  return await webcrypto.subtle.importKey("pkcs8",
    keyBuffer,
    { name: "RSA-OAEP",
      hash: "SHA-256"},
    true, ["decrypt"]);
...
export async function importSymKey(strKey: string): Promise<webcrypto.CryptoKey> {
  const keyBuffer = base64ToArrayBuffer(strKey);
  return await webcrypto.subtle.importKey("raw",
    keyBuffer,
    { name: "AES-CBC" },
    true,
    ["encrypt", "decrypt"]);
...
export async function symEncrypt(key: webcrypto.CryptoKey, data: string): Promise<string> {
  ...
  const encrypted = await webcrypto.subtle.encrypt({ name: "AES-CBC", iv: iv }, key, encodedData);

  const IVandEnc = new Uint8Array([...iv, ...new Uint8Array(encrypted)]);
  return arrayBufferToBase64(IVandEnc.buffer);
}

// Decrypt a message using a symmetric key
export async function symDecrypt(strKey: string, encryptedData: string): Promise<string> {
  const key = await importSymKey(strKey);
  const encData = new Uint8Array(base64ToArrayBuffer(encryptedData));

  const decrypted = await webcrypto.subtle.decrypt({ name: "AES-CBC", iv: encData.slice(0, 16)}, key, encData.slice(16));
